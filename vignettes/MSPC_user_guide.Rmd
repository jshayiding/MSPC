...
# Initial attemp getting desired external data from this function 
...
myDat <- print(getExtDat)

inputData <- readPeakFiles(peakFolder = myDat , pvalueBase = 1L, verbose = FALSE)
...
# This is dummy example try to make external data available for vignette code
f1 <- system.file("extdata", "bar.bed", "MSPC")
f2 <- system.file("extdata", "cat.bed", "MSPC")
f3 <- system.file("extdata", "foo.bed", "MSPC")

fAll <- list(bar=f1, cat=f2, foo=f3)

initDat <- readPeakFiles(peakFolder = fAll , pvalueBase = 1L, verbose = FALSE)
