...
stringency test. Finally, we need to do intermediate set purification before
applying BH correction test to control false discovery rate, for example, if
user choose `Technical` replicates in their input, then enriched regions passed
in one test but failed next successive test, these peaks won't be involbed in
output. To help user deeper insight about analysis result, `create_outputset`
function also provides graphical view of output.
...
provides utility function `pvalueConversion` to make data conversion of peak'
...
vignette compilation. For detailed information, please see ? `pvalueConversion`
method.
...
order of the input Chip-seq replicates. For the further detailed information, 
...
correct gemotry of overlap hit list, then using `sumlog` method from `metap`
packages to get global Fisher score. Note that obtained global fisher score is
...
This is one of key characteristic of out method that assign an enriched regions 
...
```{r 'getPlot', fig.height=8, fig.width=10}
...
peak requirement test) but failed in anohter ( failed with Fisher's combined test),
then this ERs won't be included in output. For this reason, we decided to perform 
set purification to check whether ERs are both existed in confirmed or 
discarded ERs set. Using `anti_join` method from `dplyr` packages make this easy. 
