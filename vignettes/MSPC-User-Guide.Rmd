...
total.ERs <- denoise_ERs(peakGRs = inputDat , tau.w = 1.0E-04, noiLab = "noise")
...
#lapply(confirmedERs, function(x) as(x, "GRanges"))
```
...
## Explore all discarded ERs
#lapply(discardedERs, function(x) as(x, "GRanges"))
```
...
```{r 'FDR_stats'}
## Multiple Testing Correction
mt_output <- FDR_stats(peakList = confirmedERs, 
                       pAdjustMethod = "BH", 
