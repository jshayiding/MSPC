...
export_ERs <- function(peakList_A, peakList_B, tau.s=1.0E-08) {
  ...
  allERs <-
    ...
    mutate(peakStringency = ifelse(p.value <= tau.s, "Stringent", "Weak")) %>%
    arrange(Replicate, isConfirmed, desc(peakStringency))
  res <- allERs %>% split(list(.$Replicate, .$peakStringency, .$isConfirmed))
  DF <- lapply(res, function(x) {
    x[ , -which(names(x) %in% c("isConfirmed","peakStringency", "Replicate"))]
  })
