...
#'
#' @param ovHit output of \link{filterByOverlapHit},
...
#'
#' @param isConfirmed logical vector that check whether
...
filterBycombStringency <- function(ovHit, peakset, cmbStrgThreshold,
                                   isConfirmed=c(TRUE, FALSE)){
    # sanity input param checking
    if(!"comb.pv" %in% colnames(ovHit)) {
        stop("required column is missing")
    }
    stopifnot(class(peakset)=="GRangesList")
    stopifnot(is.numeric(cmbStrgThreshold))
    confIdx <- ovHit$comb.pv <= cmbStrgThreshold
    if(!isConfirmed)
        res <- ovHit[!confIdx,]
    res <- ovHit[confIdx,]
    asGRs <- unlist(extractList(unlist(peakset, use.names = TRUE),
                                DF$subject), use.names = FALSE)

    rslt <- split(asGRs, names(asGRs))  # FIXME : redudant name in GRanges object
    return(rslt)
    #
